...
namespace SisUvex
{
	public partial class FrmMenu : Form
	{
		...
		{
			Archivo.Mixteada.FrmMixteado frm = new();
			AbrirVentanaHijo(frm);
		}

		private void reestibarToolStripMenuItem_Click(object sender, EventArgs e)
		{
			Archivo.Reestibado.FrmReestibado frm = new();
			AbrirFormularioDialog(frm, 0);
		}

		private void desestibarToolStripMenuItem_Click(object sender, EventArgs e)
		{
		...
		private void cajasPorEmpleadoDetalladoToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (!User.HasViewCatalogsPermission()) return;

			Nomina.CONTRATO.PayrollPack_BoxPerNumber.BoxPerEmployeeReport.FrmPayrollBoxPerEmployeeDetailReport cat = new();
			AbrirVentanaHijo(cat);
		}
		...
		private void registroDeBajasDePersonalToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (!User.HasViewCatalogsPermission())
				return;

			Nomina.Nom_bajas_personal.FrmEmployeeStatus cat = new();
			cat.WindowState = FormWindowState.Maximized;
			AbrirVentanaHijo(cat);
		}

		private void registroDeCuadrillasToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (!User.HasViewCatalogsPermission())
				return;
		...
		private void reporteDeAsistenciasASToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (!User.HasViewCatalogsPermission())
				return;

			Nomina.Asistencia_AS.FrmAsistenciaASConsulta cat = new();
			cat.WindowState = FormWindowState.Maximized;
			AbrirVentanaHijo(cat);
		}
		...
		private void registroDeAsistenciaToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			Nomina.NomCampoAgregarListados.FrmAsistencia cat = new();
			cat.WindowState = FormWindowState.Maximized;
			AbrirVentanaHijo(cat);
		}

		private void consultarNominaToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (!User.HasViewCatalogsPermission())
				return;

			Nomina.Nom_SemAutomaticaCampo.FrmNomina cat = new();
			cat.WindowState = FormWindowState.Maximized;
			AbrirVentanaHijo(cat);
		}
