...
{
	internal class ClsIngresosDiversos : ClsCatalogos
	{
		...
		string query = @"SELECT  
						...
						JOIN Nom_Employees emp ON emp.id_employee = lst.id_employee
						JOIN Nom_Tabulador tab ON tab.c_codigo_tab = lst.c_codigo_tab
						LEFT JOIN Nom_MiscellaneousIncome mi ON mi.id_attendence = lst.id_attendence
						LEFT JOIN Nom_concept con ON con.id_concept = mi.id_concept ";
		...
		public void InsertarIngreso()
		{
			...
			if (ExisteConceptoEnAsistencia(frmAdd.IdAttendence, idConcepto))
			{
		...
		private bool ExisteConceptoEnAsistencia(string idAttendence, string idConcepto)
		{
			string query = $@"
			SELECT COUNT(*) 
			FROM Nom_MiscellaneousIncome
			WHERE id_attendence = '{idAttendence}'";
